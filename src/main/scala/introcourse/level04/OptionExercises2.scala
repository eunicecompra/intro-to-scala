...
object OptionExercises2 {
  ...
  def findJobDescriptionGivenJobIdOrElse2(jobId: JobId): String = {
    val maybeJob: Option[String] = findJobDescriptionGivenJobId1(jobId)

    maybeJob.getOrElse(s"Job with id $jobId does not exist")
  }
  ...
  def findJobByHumanId(humanId: HumanId): Option[Job] = {
    ...
    val maybeJob: Option[Job] = maybeJobId.map(jobId => findJobById(jobId)).flatten
  ...
  def findJobNameByHumanId(humanId: HumanId): Option[String] = {
    ...
    findJobIdByHumanId(humanId).map(jobId => findJobById(jobId).map(job => job.name)).flatten
